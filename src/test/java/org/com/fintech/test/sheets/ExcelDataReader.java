package org.com.fintech.test.sheets;
...
public class ExcelDataReader implements AutoCloseable {
        ...
        if (headerRow != null) {
            for (Cell cell : headerRow) {
                if (cell != null && cell.getCellType() == CellType.STRING && !cell.getStringCellValue().trim().isEmpty()) {
                    // Converte o nome do cabeçalho para maiúsculas para garantir a correspondência sem distinção entre maiúsculas e minúsculas.
                    String headerName = cell.getStringCellValue().trim().toUpperCase();
                    indexes.put(headerName, cell.getColumnIndex());
                }
            }
        }
        return indexes;
    ...
    public Map<String, String> getRowData(String lookupColumn, String lookupValue) {
        ...
        // Busca o índice da coluna de pesquisa em maiúsculas.
        ...
        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            ...
            if (row != null) {
                ...
                // Compara os valores, tratando a possibilidade de IDs serem números no Excel ("13")
                // mas strings com zeros à esquerda na busca ("0013").
                boolean areEqual = false;
                try {
                    // Tenta comparar como números. Isso resolve "13" == "0013.0" etc.
                    if (Double.parseDouble(lookupValue) == Double.parseDouble(cellValue)) {
                        areEqual = true;
                    }
                } catch (NumberFormatException e) {
                    // Se não forem números, faz a comparação normal de strings.
                    areEqual = lookupValue.equals(cellValue);
                }
